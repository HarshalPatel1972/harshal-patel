...
function FeedbackWritingRoom({ onSend, onViewGallery, initialType }: { onSend: (name: string, msg: string, cat: string) => void, onViewGallery: () => void, initialType: string }) {
  ...
  return (
    <div className="h-screen bg-white text-black font-display p-6 md:p-10 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full flex justify-center pointer-events-none overflow-hidden z-0 opacity-5 select-none rotate-[-5deg]">
         <h2 className="text-[6rem] md:text-[22rem] font-black whitespace-nowrap leading-none tracking-tighter text-black uppercase">
            FEEDBACK
         </h2>
      </div>

      <div className="absolute top-6 left-8 z-10 flex gap-6 items-center"><Link href="/" className="font-black text-xs uppercase tracking-[0.3em] hover:opacity-50 transition-opacity">← Cancel</Link><div className="w-[1px] h-4 bg-black/20" /><button onClick={onViewGallery} className="font-black text-xs uppercase tracking-[0.3em] hover:opacity-50 transition-opacity">Enter Gallery →</button></div>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-2xl relative z-10">
        <h1 className="text-6xl md:text-7xl font-black uppercase leading-[0.85] tracking-tighter mb-10 text-center">Write Your<br /><span className="bg-black text-white px-4 inline-block mt-2 text-5xl">Message</span></h1>
        <div className="space-y-8">
          <div><label className="block text-[11px] font-black uppercase tracking-[0.2em] mb-3 opacity-40">Your Name:</label><div className="relative"><input type="text" value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Anonymous" className="w-full p-5 pr-40 border-2 border-black font-mono text-sm focus:outline-none focus:ring-0 bg-white transition-all focus:shadow-[10px_10px_0px_#000]" /><button onClick={() => setUserName(RANDOM_ID())} className="absolute right-3 top-1/2 -translate-y-1/2 px-4 py-2 bg-black text-white font-black text-[10px] uppercase tracking-widest hover:bg-[#D63031] transition-colors">RANDOMIZE</button></div></div>
          <div><label className="block text-[11px] font-black uppercase tracking-[0.2em] mb-3 opacity-40">Category:</label><div className="flex flex-wrap gap-3">{["SUBMIT REVIEW", "REPORT A BUG", "REQUEST FEATURE"].map((o) => (<button key={o} onClick={() => setType(o)} className={`px-5 py-3 text-[10px] font-black border-2 border-black transition-all ${type === o ? "bg-black text-white shadow-[6px_6px_0px_#34C759]" : "bg-transparent hover:bg-black/5"}`}>{o}</button>))}</div></div>
          <div><label className="block text-[11px] font-black uppercase tracking-[0.2em] mb-3 opacity-40">Words:</label><textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Type here..." className="w-full h-48 p-7 border-2 border-black font-mono text-sm focus:outline-none focus:ring-0 resize-none bg-white transition-all focus:shadow-[10px_10px_0px_#000]" /></div>
          <button onClick={handleSubmit} disabled={isSending || !message.trim()} className={`w-full py-7 font-black tracking-[0.4em] text-lg transition-all border-2 border-black relative ${isSending || !message.trim() ? "bg-black/10 text-black/30 opacity-50" : "bg-black text-white hover:bg-white hover:text-black hover:shadow-[12px_12px_0px_#000] hover:-translate-x-1 hover:-translate-y-1"}`}>{isSending ? "SENDING..." : "LAUNCH SIGNAL"}</button>
        </div>
      </motion.div>
    </div>
  );
