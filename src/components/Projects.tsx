      ...
      <div className={`transition-all duration-1000 relative ${isOverridden ? 'grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 opacity-100 max-w-7xl w-full' : 'flex flex-col gap-[20vh] max-w-5xl w-full md:w-[65rem] md:-mr-5 pl-0 md:pl-0'}`}>
        {!isOverridden && (
          <div className="absolute right-0 top-0 h-full w-10 md:w-12 pointer-events-none z-30">
            <div className="sticky top-[calc(10vh+60px)] md:top-[calc(12vh+60px)] pointer-events-auto flex justify-start pl-1">
              <button 
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
        ...
        {currentProjects.map((project: Project, i: number) => {
          ...
          return (
            <div 
              key={project.title} 
              className={`transition-all duration-700 ease-out w-full ${isOverridden ? `relative top-0 ${i === 0 ? "md:col-span-2" : ""}` : 'sticky top-[calc(10vh+60px)] md:top-[calc(12vh+60px)]'}`} 
