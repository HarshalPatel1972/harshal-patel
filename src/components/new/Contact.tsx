        ...
        <div className="relative w-full max-w-7xl mt-12 mb-10 border-t border-b border-[var(--sumi-ink)]/10 flex flex-col md:flex-row flex-nowrap">
          {links.map((link, i) => {
            ...
            const getLogo = (id: string) => {
              if (id === 'email') return <svg className="w-full h-full" viewBox="0 0 24 24" fill="none" stroke="#D44638" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><path d="M4 7.00005L10.2 11.65C11.2667 12.45 12.7333 12.45 13.8 11.65L20 7" /><rect x="3" y="5" width="18" height="14" rx="2" /></svg>;
            ...
            const cellContent = (
              <>
                <div className="absolute left-0 w-full h-full top-0 md:top-[100%] md:group-hover:top-0 transition-all duration-500 ease-[cubic-bezier(0.2,0.9,0.3,1)] z-0 flex items-center justify-center pointer-events-none">
                  <div className="absolute inset-0 flex items-center justify-center p-4 opacity-[0.08] md:opacity-10 scale-125">
                     {getLogo(link.id)}
                  </div>
                </div>
                <span className="relative z-10 text-[2rem] md:text-[3rem] lg:text-[4rem] font-black font-display uppercase tracking-tighter text-[var(--sumi-ink)] transition-all duration-500 ease-[cubic-bezier(0.2,0.9,0.3,1)] md:group-hover:text-[var(--forge-orange)] md:group-hover:-translate-y-2 pointer-events-auto">
                  {copied && link.id === 'email' ? 'COPIED!' : link.label}
                </span>
              </>
            );

            const className = `relative flex-1 flex items-center justify-center py-10 md:py-16 overflow-hidden cursor-pointer group outline-none ${isLast ? '' : 'border-b md:border-b-0 md:border-r border-[var(--sumi-ink)]/10'}`;
