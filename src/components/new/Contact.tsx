  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="relative w-full max-w-7xl mt-12 mb-10 border-t border-b border-[var(--sumi-ink)]/10 flex flex-row flex-nowrap">
          {links.map((link, i) => {
            ...
            const cellContent = (
              <>
                <div className="absolute left-0 w-full h-full top-[100%] group-hover:top-0 transition-all duration-500 ease-[cubic-bezier(0.2,0.9,0.3,1)] z-0 flex items-center justify-center pointer-events-none">
                  <div className="absolute inset-0 flex items-center justify-center p-4 opacity-10 scale-125">
                     {getLogo(link.id, 'var(--forge-orange)')}
                  </div>
                </div>
                <span className="relative z-10 text-[2rem] md:text-[3rem] lg:text-[4rem] font-black font-display uppercase tracking-tighter text-[var(--sumi-ink)] transition-all duration-500 ease-[cubic-bezier(0.2,0.9,0.3,1)] group-hover:text-[var(--forge-orange)] group-hover:-translate-y-2 pointer-events-auto">
                  {copied && link.id === 'email' ? 'COPIED!' : link.label}
                </span>
              </>
            );

            const className = `relative flex-1 flex items-center justify-center py-10 md:py-16 overflow-hidden cursor-pointer group outline-none ${isLast ? '' : 'border-r border-[var(--sumi-ink)]/10'}`;
