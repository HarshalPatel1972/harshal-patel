...
export function Hero() {
  ...
  const layoutCacheRef = useRef({ offsetTop: 0, trackHeight: 0 });
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const { offsetTop: sectionOffset, trackHeight } = layoutCacheRef.current;
      
      if (trackHeight <= 0 || st > sectionOffset + trackHeight + 500) return;
