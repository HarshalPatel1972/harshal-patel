...
function ContactLinkItem({ link, language, copied, setCopied, isLast }: any) {
  ...
  return (
    <a
      ...
      className={`relative flex-1 flex items-center justify-center py-10 md:py-16 overflow-hidden cursor-pointer group outline-none ${isLast ? '' : 'border-b md:border-b-0 md:border-r border-[#e5e5e5]'}`}
    >
      <div className="absolute left-0 w-full h-full top-0 md:top-[100%] md:group-hover:top-0 transition-all duration-500 ease-[cubic-bezier(0.2,0.9,0.3,1)] z-0 flex items-center justify-center pointer-events-none">
        <div className="absolute inset-0 flex items-center justify-center p-4 opacity-[0.08] md:opacity-15 scale-125">
           {getLogo(link.id)}
        </div>
      </div>
      <span className="relative z-10 text-[2rem] md:text-[3rem] lg:text-[4rem] font-black font-display uppercase tracking-tighter text-[var(--bg-ink)] transition-all duration-500 ease-[cubic-bezier(0.2,0.9,0.3,1)] md:group-hover:text-[var(--accent-blood)] md:group-hover:-translate-y-2 pointer-events-auto">
        {textValue}
      </span>
    </a>
  );
