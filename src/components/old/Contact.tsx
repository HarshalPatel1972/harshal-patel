...
function ContactLinkItem({ link, language, copied, setCopied, isLast }: any) {
  ...
  const getLogo = (id: string) => {
    if (id === 'email') return (
      <svg preserveAspectRatio="xMidYMid slice" className="absolute inset-0 w-full h-full object-cover" viewBox="52 42 88 66">
  ...
  return (
    <a
      ...
      className={`relative flex-1 flex items-center justify-center py-12 md:py-20 overflow-hidden cursor-pointer group outline-none ${isLast ? '' : 'border-b md:border-b-0 md:border-r border-[#e5e5e5]'}`}
    >
      ...
      <span className="relative z-10 text-[3rem] sm:text-[5rem] md:text-[6.5rem] lg:text-[8rem] font-black font-display uppercase tracking-tighter text-[var(--bg-ink)] transition-all duration-500 ease-[cubic-bezier(0.2,0.9,0.3,1)] md:group-hover:text-[var(--accent-blood)] md:group-hover:-translate-y-2 pointer-events-auto leading-none">
        {textValue}
      </span>
    </a>
  );
