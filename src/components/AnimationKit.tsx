...
export function useMagnetic(strength: number = 0.3) {
  ...
  useEffect(() => {
    ...
    // Bolt: Performance optimization - use ticking and rAF to prevent layout thrashing
    let ticking = false;
    let currentX = 0;
    let currentY = 0;
    let rafId: number;
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      currentY = e.clientY;
      if (!ticking) {
        rafId = window.requestAnimationFrame(updateMagnetic);
        ticking = true;
      }
    };
...
export function useParallax(speed: number = 0.3) {
  ...
  useEffect(() => {
    ...
    // Bolt: Performance optimization - use ticking and rAF to prevent layout thrashing on scroll
    let ticking = false;
    let rafId: number;
    ...
    <div className={`fixed bottom-4 right-[64px] md:bottom-8 md:right-[112px] z-[50] pointer-events-none mix-blend-difference text-white flex flex-col items-end leading-none select-none transition-opacity duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
       <div className="relative flex flex-col items-end">
         ...
         <img 
           ...
           className="w-[208px] md:w-[606px] -mb-[8px] md:-mb-[32px] mr-[5px] md:mr-[20px] translate-x-[32px] translate-y-[30px] md:translate-x-[97px] md:translate-y-[90px] z-20 pointer-events-none select-none"
         />
         {/* The 000 Text Block */}
