...
// Hoisted static arrays for performance
// Pre-calculating these avoids repetitive object allocation on every render
...
const STATIC_CHAOTIC_TEXT = Array.from({ length: 100 }).map((_, i) => ({
  id: i,
    ...
    <div className="absolute inset-0 flex items-center justify-center z-0 pointer-events-none overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[#050505]">
        <div className="absolute inset-0 opacity-20 flex flex-wrap gap-4 p-8 overflow-hidden font-mono text-[10px] text-[var(--accent-blood)] select-none">
          {STATIC_CHAOTIC_TEXT.map((item) => (
            <span key={item.id} className="animate-pulse" style={{ animationDelay: item.delay }}>
              {item.text}
            </span>
          ))}
        </div>
