...
// Static arrays hoisted outside component to improve performance
...
const STATIC_TEXTS = Array.from({ length: 100 }).map((_, i) => ({
  text: ["0xDEBT", "FATAL_ERR", "NULL_PTR", "MEM_LEAK", "DEPRECATED"][i % 5],
  delay: Math.random() * 2
}));

const STATIC_GRID_LINES = Array.from({ length: 20 }).map((_, i) => ({
  pos: 220 + i * 30
}));
    ...
    <div className="absolute inset-0 flex items-center justify-center z-0 pointer-events-none overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[#050505]">
        <div className="absolute inset-0 opacity-20 flex flex-wrap gap-4 p-8 overflow-hidden font-mono text-[10px] text-[var(--accent-blood)] select-none">
          {STATIC_TEXTS.map((item, i) => (
            <span key={i} className="animate-pulse" style={{ animationDelay: `${item.delay}s` }}>
              {item.text}
            </span>
          ))}
        </div>
            ...
            <g className="opacity-20">
              {STATIC_GRID_LINES.map((line, i) => (
                <React.Fragment key={i}>
                  <line x1={line.pos} y1="0" x2={line.pos} y2="1000" stroke="white" strokeWidth="0.5" />
                  <line x1="0" y1={line.pos} x2="1000" y2={line.pos} stroke="white" strokeWidth="0.5" />
                </React.Fragment>
              ))}
            </g>
