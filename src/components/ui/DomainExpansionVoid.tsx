...
// Static properties hoisted outside the component to prevent re-calculations and impure Math.random() during renders
...
const STATIC_CHAOS_TEXTS = Array.from({ length: 100 }).map((_, i) => ({
  id: i,
  text: ["0xDEBT", "FATAL_ERR", "NULL_PTR", "MEM_LEAK", "DEPRECATED"][i % 5],
  delay: Math.random() * 2
}));

const STATIC_GRID_LINES = Array.from({ length: 20 }).map((_, i) => i);
    ...
    <div className="absolute inset-0 flex items-center justify-center z-0 pointer-events-none overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[#050505]">
        <div className="absolute inset-0 opacity-20 flex flex-wrap gap-4 p-8 overflow-hidden font-mono text-[10px] text-[var(--accent-blood)] select-none">
          {STATIC_CHAOS_TEXTS.map((item) => (
            <span key={item.id} className="animate-pulse" style={{ animationDelay: `${item.delay}s` }}>
              {item.text}
            </span>
          ))}
        </div>
