...
// Generate random shards statically outside render cycle
...
// Pre-calculate exterior text elements
const exteriorElements = Array.from({ length: 100 }).map((_, i) => ({
  text: ["0xDEBT", "FATAL_ERR", "NULL_PTR", "MEM_LEAK", "DEPRECATED"][i % 5],
  delay: `${Math.random() * 2}s`
}));

// Pre-calculate architectural blueprint lines
const blueprintLines = Array.from({ length: 20 }).map((_, i) => ({
  vX: 220 + i * 30,
  hY: 220 + i * 30
}));
    ...
    <div className="absolute inset-0 flex items-center justify-center z-0 pointer-events-none overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[#050505]">
        <div className="absolute inset-0 opacity-20 flex flex-wrap gap-4 p-8 overflow-hidden font-mono text-[10px] text-[var(--accent-blood)] select-none">
          {exteriorElements.map((el, i) => (
            <span key={i} className="animate-pulse" style={{ animationDelay: el.delay }}>
              {el.text}
            </span>
          ))}
        </div>
