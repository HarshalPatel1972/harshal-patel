...
// Pre-compute static arrays outside render cycle to avoid recreation and useMemo overhead
...
const STATIC_DEBT_ITEMS = Array.from({ length: 100 }).map((_, i) => ({
  id: i,
  delay: Math.random() * 2,
  text: ["0xDEBT", "FATAL_ERR", "NULL_PTR", "MEM_LEAK", "DEPRECATED"][i % 5]
}));

const STATIC_GRID_LINES = Array.from({ length: 20 });
