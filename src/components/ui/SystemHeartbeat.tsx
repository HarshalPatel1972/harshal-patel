...
// Pre-calculate random network paths at module level to avoid react-hooks/purity lint errors
// and improve performance by not re-calculating on every component mount.
const NERVES = Array.from({ length: 15 }).map(() => ({
  x1: Math.random() * 1000,
  y1: Math.random() * 1000,
  x2: Math.random() * 1000,
  y2: Math.random() * 1000,
...
const GRID_LINES = Array.from({ length: 20 }).map((_, i) => i);
