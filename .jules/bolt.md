...
## 2024-05-18 - Preventing Layout Thrashing in `requestAnimationFrame` Loops
**Learning:** Reading DOM dimensions synchronously (`clientWidth`, `clientHeight`, `getBoundingClientRect`) inside a `requestAnimationFrame` loop—especially within an inner loop over multiple elements—forces synchronous layout recalculations (Layout Thrashing). This severely bottlenecks main-thread performance and drops frame rates during continuous animations, as seen in the `SkillGlobe` 3D rendering component.
**Action:** Extract DOM dimension reads out of the `requestAnimationFrame` callback. Cache the dimensions (e.g., width, height) in outer variables or refs on component mount, and update them asynchronously via a `ResizeObserver` on the container element. Read exclusively from these cached values inside the frame loop.
